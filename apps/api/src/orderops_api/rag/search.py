...
from orderops_api.rag.rerank import build_reranker
...
def search_policy(query: str, top_k: int = 5, rerank: bool = True) -> list[PolicySearchResult]:
    ...
    query_vector = embedding_provider.embed_text(query, input_type="query")
    ...
    return build_reranker(settings, enabled=rerank).rerank(query, results)[:top_k]
